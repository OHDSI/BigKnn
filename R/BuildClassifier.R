...
buildKnn <- function(outcomes,
  ...
  for (i in bit::chunk(nonZeroOutcomeRowIds, by = 1e+05)) {
    knn$addNonZeroOutcomes(rJava::.jarray(as.numeric(nonZeroOutcomeRowIds[i])))
  }
  chunks <- bit::chunk(covariates, by = 1e+05)
  pb <- txtProgressBar(style = 3)
  for (i in 1:length(chunks)) {
    knn$addCovariates(rJava::.jarray(as.numeric(covariates$rowId[chunks[[i]]])),
                      rJava::.jarray(as.numeric(covariates$covariateId[chunks[[i]]])),
                      rJava::.jarray(as.numeric(covariates$covariateValue[chunks[[i]]])))
